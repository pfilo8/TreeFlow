...
from torch.distributions import Normal, NegativeBinomial, Poisson, StudentT, Laplace, Uniform, TransformedDistribution, \
    SigmoidTransform, AffineTransform, Gamma, Gumbel, Weibull, ExpTransform
...
class PGBM(object):
    ...
    def train(self, train_set, objective, metric, params=None, valid_set=None, sample_weight=None,
        ...
        for estimator in range(start_iteration, self.n_estimators + start_iteration):
            ...
            if validate:
                ...
                if self.verbose > 1:
                    print(
                        f"Estimator {estimator}/{self.n_estimators + start_iteration}, Train metric: {train_metric:.4f}, Validation metric: {validation_metric:.4f}")
                ...
                if self.verbose > 1:
                    print(
                        f"Estimator {estimator}/{self.n_estimators + start_iteration}, Train metric: {train_metric:.4f}")
                self.best_iteration = estimator + 1
    ...
    def predict_dist(self, X, n_forecasts=100, parallel=True, output_sample_statistics=False):
        ...
        if self.distribution == 'normal':
            ...
            yhat = TransformedDistribution(base_dist, [SigmoidTransform().inv, AffineTransform(loc, scale)]).rsample(
                [n_forecasts])
            ...
            def weibull_params_grad(k):
                ...
                hess = -(2 * gamma(2 / k) * (
                        2 * psi0(2 / k) ** 2 - 2 * k * psi0(1 / k) - psi1(1 / k) + 2 * k * psi0(2 / k) - 4 * psi0(
                    1 / k) * psi0(2 / k) + 2 * psi1(2 / k) + 2 * psi0(1 / k) ** 2)) / (k ** 4 * gamma(1 / k) ** 2)
    ...
    def nll(self, X, y, parallel=True):
        """
        Calculate negative loglikelihood for a given sample set X
        """
        X = self._convert_array(X)
        y = torch.Tensor(y)
        initial_estimate = self.initial_estimate.repeat(X.shape[0])
        ...
        if self.distribution == 'normal':
            ...
            scale = torch.nan_to_num(((3 * variance) / np.pi ** 2).sqrt(), 1e-9)
            base_dist = Uniform(torch.zeros(X.shape[0], device=X.device), torch.ones(X.shape[0], device=X.device))
            dist = TransformedDistribution(base_dist, [SigmoidTransform().inv, AffineTransform(loc, scale)])
        elif self.distribution == 'lognormal':
            mu_adj = mu.clamp(1e-9)
            variance = torch.nan_to_num(variance, 1e-9).clamp(1e-9)
            loc = torch.log(mu_adj ** 2 / torch.sqrt(variance + mu_adj ** 2))
            scale = torch.log(1 + variance / mu_adj ** 2).clamp(1e-9)
            dist = TransformedDistribution(Normal(loc, torch.sqrt(scale)), ExpTransform())
        elif self.distribution == 'gamma':
            variance = torch.nan_to_num(variance, 1e-9)
            mu_adj = torch.nan_to_num(mu, 1e-9)
            rate = (mu_adj.clamp(1e-9) / variance.clamp(1e-9))
            ...
            probs = (1 - (loc / scale)).clamp(0, 0.99999)
            counts = (-loc ** 2 / (loc - scale)).clamp(eps)
            dist = NegativeBinomial(counts, probs)
        elif self.distribution == 'poisson':
            dist = Poisson(mu.clamp(1e-9))
        else:
            print('Distribution not (yet) supported')
